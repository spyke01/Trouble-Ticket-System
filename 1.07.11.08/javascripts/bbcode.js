...
bbcode = new Array();
bbtags = new Array('[b]','[/b]','[i]','[/i]','[u]','[/u]','[quote]','[/quote]','[code]','[/code]','[list]','[/list]','[list=]','[/list]','[img]','[/img]','[url]','[/url]','[email]','[/email]','[php]','[/php]','[align=center]','[/align]','[align=left]','[/align]','[align=right]','[/align]');
imageTag = false;

// Shows the help messages in the helpline window
function helpline(help) {
	document.newpageform.helpbox.value = eval(help + "_help");
}
...
function checkForm() {
	...
	if (document.newpageform.pagecontent.value.length < 2) {
...
function bbplace(text) {
	var txtarea = document.newpageform.pagecontent;
...
function bbstylebutton(bbnumber) {
	...
	if (bbnumber == -1) { // Close all open tags & default button names
		while (bbcode[0]) {
			...
			buttext = eval('document.newpageform.addbbcode' + butnumber + '.value');
			eval('document.newpageform.addbbcode' + butnumber + '.value ="' + buttext.substr(0,(buttext.length - 1)) + '"');
		}
	...
	if (donotinsert) {		// Close all open tags up to the one just clicked & default button names
		...
		if (imageTag && (bbnumber != 14)) {		// Close image tag before adding another
			...
			document.newpageform.addbbcode14.value = "Img";	// Return button back to normal state
			imageTag = false;
		}
		...
		eval('document.newpageform.addbbcode'+bbnumber+'.value += "*"');
		txtarea.focus();
		return;
	}
	storeCaret(txtarea);
